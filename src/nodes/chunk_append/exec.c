	...
	foreach (lc, state->filtered_subplans)
	{
		...
		if (osm_chunk_exclusion_hook)
		{
			...
			if (chunk && IS_OSM_CHUNK(chunk))
			{
				...
				if (IsA(state->subplanstates[i], SortState) &&
					IsA(((SortState *) state->subplanstates[i])->ss.ps.lefttree, ForeignScanState))
					fsstate = (ForeignScanState *) ((SortState *) state->subplanstates[i])
								  ->ss.ps.lefttree;
